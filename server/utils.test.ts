...
(async () => {
  ...
  const audioTests = [
    ...
    },
    {
      ...
      input: Buffer.from("FFF30000", "hex"),
      expected: true
    },
    {
      name: "Valid WAV",
    ...
    {
      name: "Valid OGG",
      input: Buffer.from("4F676753", "hex"),
      expected: true
    },
    {
      name: "Valid FLAC",
      input: Buffer.from("664C6143", "hex"),
      expected: true
    },
    {
    ...
    },
    {
      ...
      input: Buffer.from("This is a text file", "utf8"),
      expected: false
    },
    {
    ...
    },
    {
      ...
      input: Buffer.from("01020304", "hex"),
      expected: false
    }
  ];
  ...
  for (const test of audioTests) {
    try {
      const result = await verifyAudioFileSignature(test.input);
      assert.strictEqual(result, test.expected, test.name);
      console.log(`✅ ${test.name}`);
    } catch (err: any) {

...
export class DatabaseStorage implements IStorage {
  ...
  async getPlaylistWithSongs(id: number): Promise<(Playlist & { songs: Song[] }) | undefined> {
    ...
    // Optimized: Use single query with innerJoin and order by join table ID
    // to prevent N+1 queries, eliminate manual map reconstruction, and ensure
    // songs are ordered chronologically by insertion.

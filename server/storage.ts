...
export class DatabaseStorage implements IStorage {
  ...
  async toggleLike(userId: string, songId: number): Promise<{ liked: boolean; likeCount: number }> {
    return await db.transaction(async (tx) => {
      // Check if song exists first (optimized: only select ID to avoid fetching full object)
      const [songExists] = await tx.select({ id: songs.id }).from(songs).where(eq(songs.id, songId));
      if (!songExists) return { liked: false, likeCount: 0 };
      ...
      if (existingLike) {
        ...
        // Atomic decrement using SQL to ensure data integrity
        ...
        // Atomic increment using SQL to ensure data integrity
        const [updatedSong] = await tx.update(songs)
